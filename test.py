...
@st.cache_data
def process_data(rows, resample_freq='D', columns_to_check=None):
    """Process data by converting to DataFrame, filtering, and resampling."""
    ...
    with st.spinner("Generating Charts...please wait"):
        ...
        try:
            ...
            with ThreadPoolExecutor() as executor:
                indoor_future = executor.submit(fetch_data, indoor_query_month, (device_id, year, selected_month))
                indoor_year_future = executor.submit(fetch_data, indoor_query_year, (device_id, year))
                outdoor_future = executor.submit(fetch_data, outdoor_query, (outdoor_device_id, year, selected_month))

                indoor_rows = indoor_future.result()
                indoor_rows_year = indoor_year_future.result()
                outdoor_rows = outdoor_future.result()
            ...
            if indoor_rows and outdoor_rows:
                ...
                st.markdown("<h3 style='text-align:center;'>Line Charts of Indoor & Outdoor</h3>", unsafe_allow_html=True)
            ...
            if indoor_rows_year:
                indoor_df_year = process_data(indoor_rows_year, resample_freq='D')
                st.markdown("<h3 style='text-align:center;'>Seasonal Line Chart for Residential Buildings</h3>", unsafe_allow_html=True)
